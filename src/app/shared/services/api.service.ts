...
import { Book, BookCategory, Order, User, UserType } from 'src/app/models/models';

@Injectable({
  providedIn: 'root'
})
export class ApiService {
  baseUrl: string = "https://localhost:7027/api/Library/";
  ...
  login(info: any){
    let params = new HttpParams().append('email',info.email)
  ...
  approveRequest(userId: number){
    return this.http.get(this.baseUrl + 'ApproveRequest', {
      params: new HttpParams().append('userId',userId),responseType: 'text',
    });
  }
